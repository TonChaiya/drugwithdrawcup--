...
function filterRows(inputId,rowClass){const q=document.getElementById(inputId).value.trim().toLocaleLowerCase('th');document.querySelectorAll('.'+rowClass).forEach(row=>{row.hidden=q!==''&&!row.dataset.search.includes(q);if(rowClass==='facilityRow'&&row.hidden)row.querySelector('.facilityCheck').checked=false});if(rowClass==='facilityRow'){zoneEl('facilityAll').checked=false;updateBulkButton()}}
